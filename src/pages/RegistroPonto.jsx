...
import { API_ENDPOINTS } from "../api"; // importei só o que usa
...
export default function RegistroPonto() {
  ...
  const buscarPontos = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_ENDPOINTS.listarPontos}?user_id=${user.id}&mes=${mes}&ano=${ano}`);
      const data = await res.json();
      setRegistros(data);
    } catch {
  ...
  useEffect(() => {
    async function buscarCargaHoraria() {
      if (!user?.id) return;
      try {
        const res = await fetch(`${API_ENDPOINTS.cargaHoraria}?user_id=${user.id}`);
        const data = await res.json();
        if (data?.carga != null) {
  ...
  const registrarPonto = async () => {
    ...
    try {
      const res = await fetch(API_ENDPOINTS.registrarPonto, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          user_id: user.id,
  ...
  // Funções auxiliares para formatar horas (sem alteração)
  ...
  // JSX permanece igual, só URLs atualizadas acima
  ...
  return (
    <div style={{ padding: "1rem", fontFamily: "Arial", position: "relative" }}>
      ...
      <section style={{ maxWidth: "300px", margin: "auto", marginTop: "20px" }}>
        ...
        >
          <p>
            <strong>Carga horária do mês:</strong>{" "}
            {cargaMensal != null ? `${cargaMensal}h` : "-"}
          </p>
          <p>
            <strong>Horas executadas:</strong>{" "}
            {toHoursMinutesFromDecimal(horasExecutadas)}
          </p>
          <p>
            <strong>Horas a cumprir:</strong>{" "}
            {cargaMensal != null
