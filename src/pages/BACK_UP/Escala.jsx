  ...
  const diasDoMes = useMemo(() => {
    ...
    return Array.from({ length: diasNoMes }, (_, i) => {
      ...
      return {
        ...
        semana: format(data, "EEE", { locale: ptBR }),
        ...
        <Table variant="simple" size="sm">
          <Thead position="sticky" top={0} bg="gray.100" zIndex={10}>
            <Tr>
              <Th>Usuário</Th>
              {diasDoMes.map((d) => (
                <Th key={d.dia} fontSize="xs" textAlign="center" color={["sáb", "dom"].includes(d.semana.toLowerCase()) ? "red.500" : "black"}>
                  {d.dia}<br />{d.semana}
                </Th>
              ))}
            </Tr>
        ...
        <Td colSpan={diasDoMes.length + 1}>
          <Box border="1px solid #ccc" borderRadius="md" p={3} mt={2}>
            ...
            <Button
              ...
              mr={2}
              onClick={() =>
                preencherEscalaRapida(user.id, "par", escalaRapida[user.id])
              }
            >
              Dias Pares
            </Button>
            <Button
              ...
              mr={2}
              onClick={() =>
                preencherEscalaRapida(user.id, "impar", escalaRapida[user.id])
              }
            >
              Dias Ímpares
            </Button>
            <Button
              ...
              colorScheme="purple"
              mr={2}
              onClick={() => preencherDiasUteis(user.id, escalaRapida[user.id])}
            >
              Dias Úteis
            </Button>
            <Button
